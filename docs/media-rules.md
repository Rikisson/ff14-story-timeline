...
- Uploads enforce a max file size on every kind, and a minimum source width on covers and backgrounds, before write. Oversize dimensions are downscaled — not rejected — so the user's first try succeeds; truly broken inputs (wrong mime, unreadable, or a cover/background below the width floor) surface a clear error. Per-kind targets: cover and background scale to fit within 2560×1440 (16:9) with a 1280px-wide floor; sprites scale to fit within 1440×2560 (9:16) — the same pixel envelope rotated to portrait.
- Every image upload opens an in-browser crop step before the asset is created. The crop dialog opens at the aspect ratio the kind expects — 16:9 for covers and backgrounds, 9:16 for sprites — and also offers 1:1 and free-form; the author can reframe, switch ratios, or skip cropping entirely with "Use full image". Covers and backgrounds carry a crop-time 1280px-wide floor so the framed result still clears the upload minimum. The cropper is a standalone, reusable component wired once into the shared asset picker, so it covers every image-upload surface; the crop is re-encoded losslessly and preserves sprite transparency. Audio uploads skip it.
