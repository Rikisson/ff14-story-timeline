...
// ---------------------------------------------------------------------------
// applyCategoryRename — preserves key, validates uniqueness
// ---------------------------------------------------------------------------

describe('applyCategoryRename', () => {
  it('updates label/color/description while preserving the key', async () => {
    withConfig({
      categories: [{ id: 'c1', key: 'items', label: 'Items', color: '#aaa' }],
      version: 1,
    });

    await runTx((tx) =>
      applyCategoryRename(tx as never, FAKE_FIRESTORE, 'u1', 'c1', {
        label: 'Items — Equipment',
        color: '#bbb',
      }),
    );

    const written = recordedOps.find((o) => o.path === CONFIG_PATH)!.data as unknown as CodexCategoriesConfig;
  ...
  it('throws on a folded-label clash with another existing category', async () => {
    withConfig({
      categories: [
        { id: 'c1', key: 'items', label: 'Items' },
        { id: 'c2', key: 'lore', label: 'Lore' },
      ],
      version: 1,
    });
    ...
    await expect(
      runTx((tx) =>
        applyCategoryRename(tx as never, FAKE_FIRESTORE, 'u1', 'c2', { label: 'ITEMS' }),
      ),
    ).rejects.toThrow(CategoryConflictError);
  });

  it('allows a no-op rename to the same label without throwing', async () => {
    withConfig({
      categories: [{ id: 'c1', key: 'items', label: 'Items' }],
      version: 1,
    });

    await runTx((tx) =>
      applyCategoryRename(tx as never, FAKE_FIRESTORE, 'u1', 'c1', { label: 'Items' }),
    );

    const written = recordedOps.find((o) => o.path === CONFIG_PATH)!.data as unknown as CodexCategoriesConfig;
    expect(written.categories[0].label).toBe('Items');
  });

  it('throws when the target category id is missing', async () => {
    withConfig({ categories: [{ id: 'c1', key: 'items', label: 'Items' }], version: 1 });
    await expect(
      runTx((tx) =>
        applyCategoryRename(tx as never, FAKE_FIRESTORE, 'u1', 'unknown', { label: 'X' }),
      ),
...
describe('applyCategoryDelete', () => {
  it('removes the category and bumps the version', async () => {
    withConfig({
      categories: [
        { id: 'c1', key: 'items', label: 'Items' },
        { id: 'c2', key: 'lore', label: 'Lore' },
    ...
    await runTx((tx) =>
      applyCategoryDelete(tx as never, FAKE_FIRESTORE, 'u1', 'c1'),
    );

    const written = recordedOps.find((o) => o.path === CONFIG_PATH)!.data as unknown as CodexCategoriesConfig;
    expect(written.categories.map((c) => c.id)).toEqual(['c2']);
    expect(written.version).toBe(5);
  });

  it('is a no-op (no write) when the id is unknown', async () => {
    withConfig({
      categories: [{ id: 'c1', key: 'items', label: 'Items' }],
      version: 1,
    });
    ...
    await runTx((tx) =>
      applyCategoryDelete(tx as never, FAKE_FIRESTORE, 'u1', 'unknown'),
    );

    expect(recordedOps.find((o) => o.path === CONFIG_PATH)).toBeUndefined();
  });
});
