...
import { CodexEntry, CodexEntryDraft, StoredCodexEntry } from './codex-entry.types';

const PAGE_SIZE = 50;

@Injectable({ providedIn: 'root' })
export class CodexEntriesService {
  private readonly firebase = inject(FirebaseService);
  private readonly universes = inject(UniverseStore);
  private readonly isBrowser = isPlatformBrowser(inject(PLATFORM_ID));

  private readonly _entries = signal<CodexEntry[]>([]);
  readonly entries: Signal<CodexEntry[]> = this._entries.asReadonly();
    ...
    if (this.isBrowser) {
      effect(() => {
        ...
        this.refresh(id).catch((err) => {
          console.error('CodexEntriesService.refresh failed', err);
  ...
  async refresh(universeId?: string): Promise<void> {
    ...
    const q = query(
      collection(this.firebase.firestore, 'universes', id, 'codexEntries'),
      orderBy('createdAt', 'desc'),
      limit(PAGE_SIZE),
    );
    const snap = await getDocs(q);
    this._entries.set(snap.docs.map((d) => ({ id: d.id, ...(d.data() as StoredCodexEntry) })));
  }

  async create(draft: CodexEntryDraft, authorUid: string): Promise<string> {
    const universeId = this.requireUniverseId();
    await this.assertSlugAvailable(universeId, draft.slug);
    const id = crypto.randomUUID();
    const data: StoredCodexEntry = {
      ...draft,
      authorUid,
      createdAt: Date.now(),
    };
    await setDoc(doc(this.firebase.firestore, 'universes', universeId, 'codexEntries', id), data);
    await this.refresh(universeId);
    return id;
  }

  async update(id: string, patch: CodexEntryDraft): Promise<void> {
    const universeId = this.requireUniverseId();
    await this.assertSlugAvailable(universeId, patch.slug, id);
    await updateDoc(
      doc(this.firebase.firestore, 'universes', universeId, 'codexEntries', id),
  ...
  async remove(id: string): Promise<void> {
    const universeId = this.requireUniverseId();
    await deleteDoc(doc(this.firebase.firestore, 'universes', universeId, 'codexEntries', id));
    await this.refresh(universeId);
  }
  ...
  private async assertSlugAvailable(
    ...
    if (taken) throw new SlugTakenError('codexEntry', slug);
  }
