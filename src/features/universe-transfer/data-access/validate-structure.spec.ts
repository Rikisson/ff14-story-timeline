...
describe('validateStructure', () => {
  ...
  it('flags a character missing its name', () => {
    expect(
      codes(validateStructure({ formatVersion: 1, characters: [{ slug: 'eldrin' }] })),
    ).toContain('field-required');
  });

  it('flags an invalid slug', () => {
    expect(
      codes(validateStructure({ formatVersion: 1, characters: [{ slug: 'Eldrin!', name: 'E' }] })),
  ...
  it('flags a story whose startScene is not defined', () => {
    const issues = validateStructure({
      formatVersion: 1,
      stories: [
        {
    ...
    expect(codes(issues)).toContain('start-scene-unknown');
  });
