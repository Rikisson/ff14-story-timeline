    ...
    <ng-container *transloco="let t; prefix: 'catalog'">
      ...
      >
        ...
        <div class="flex gap-2 border-t border-surface-muted px-4 py-3">
          <a uiPrimary [routerLink]="['/play', story().id]" class="flex-1">{{ t('action.play') }}</a>
          @if (canEdit()) {
            <a uiSecondary [routerLink]="['/edit', story().id]" class="flex-1">{{ t('action.edit') }}</a>
            <button
              uiGhost
              type="button"
...
export class CatalogCardComponent {
  ...
  protected confirmDelete(): void {
    ...
    if (ok) this.remove.emit(this.story().id);
  }
